...
class Model(AttentionModel, DAModel):
    ...
    def _build_history(self, emb, rank=0, dtype=tf.int32):
        ...
        """
        self.prev_embs = tf.get_variable(
            "prev_embs",
            [0, self.hparams.decoder_num_units], dtype=dtype, trainable=False)
        self.prev_dlg_ids = tf.get_variable(
            "prev_dlg_ids", [0], dtype=tf.string,
